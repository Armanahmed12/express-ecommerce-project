...
import { zodProValidationSchema } from "./product.validation";

const createNewProduct = async(req: Request, res: Response) => {
   try {
     const userGivenData = req.body;
     
    const result = zodProValidationSchema.safeParse(userGivenData);
    ...
    if (!result.success) {
       ...
       const createdData = await ProductServices.createNewProductIntoDB(result.data);
       res.status(200).json({
         success: true,
  ...
  try {
        ...
        // if there is searchTerm 
         if (searchTerm) {
           ...
           const foundDocsBySearchTerm = await ProductServices.findProductsByQueryTextFromDB(searchTerm);
           res.status(200).json({
             success: true,
             message:
...
const getSpecificProById = async (req: Request, res: Response) => {
     ...
     try {
         const result = await ProductServices.getSpecificProByIdFromDB(req.params.productId);
         res.status(200).json({
           success: true,
